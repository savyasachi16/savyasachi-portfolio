export const config = {
  name: {
    first: 'Savyasachi',
    last: 'Jagadeeshan',
    nickname: 'Savy',
  },
  titles: ['Technical Lead', 'Senior Software Engineer', 'Backend Engineer'],
  bio: 'Been building backend systems and APIs for 8 years. Currently in the Bay Area.',
  siteUrl: 'https://folio-rouge-chi.vercel.app',
  metaDescription: 'Software Engineer with 8 years of experience and a focus on Backend Engineering. Based in San Jose, CA.',
  social: {
    github: 'https://github.com/savyasachi16',
    linkedin: 'https://linkedin.com/in/jsavyasachi/',
    email: 'jsavyasachi@gmail.com',
  },
  goodreadsUserId: '62623007',
};
